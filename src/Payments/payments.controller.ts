...
export class MpesaController {
  ...
  initiatePayment = async (c: Context) => {
    try {
      ...
      const existingPayment = await db
        ...
        .where(
          and(
            eq(payments.bookingId, parseInt(bookingId)),
   ...
   */
  queryPaymentStatus = async (c: Context) => {
    try {
   ...
   */
  getPaymentHistory = async (c: Context) => {
    try {
      const bookingId = c.req.param('bookingId');
   ...
   */
  getAllPayments = async (c: Context) => {
    try {
  ...
  retryPayment = async (c: Context) => {
    try {
      ...
      await db
        ...
        .where(
          and(
            eq(mpesaRequests.bookingId, parseInt(bookingId)),
            eq(mpesaRequests.status, 'pending')
          )
        );
